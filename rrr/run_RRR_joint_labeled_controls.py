...
version = 'Joint_labeled_Controls_%s_%s' % (params['direction'],'behavout' if params['regress_behavout'] else 'original')
...
R2_cv               = np.full((nvaluefields,narealabelpairs+1,nSessions,params['nStim']),np.nan) #dim 0: 3 = allneurons, V1unl, V1lab separately
            ...
            for istim,stim in enumerate(np.unique(ses.trialdata['stimCond'])): # loop over orientations 
                ...
                for ikf, (idx_train, idx_test) in enumerate(kf.split(X)):
                    ...
                    for r in range(nranks):
                        ...
                        R2_ranks[ivaluematch,0,ises,istim,r,imf,ikf] = EV(Y_test,Y_hat_test_rr)
                        ...
                        R2_ranks[ivaluematch,1,ises,istim,r,imf,ikf] = EV(Y_test,Y_hat_test_rr)
                        ...
                        R2_ranks[ivaluematch,2,ises,istim,r,imf,ikf] = EV(Y_test,Y_hat_test_rr)
        ...
        if np.any(~np.isnan(R2_ranks[0][0][ises])):
            for istim in range(params['nStim']):
                if fixed_rank is not None:
                    rank = fixed_rank
                    R2_cv[ivaluematch,0,ises,istim] = np.nanmean(R2_ranks[ivaluematch,0,ises,istim,rank,:,:])
