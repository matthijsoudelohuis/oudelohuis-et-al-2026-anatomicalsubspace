...
for ises in range(nSessions):
    ...
    sessions[ises].load_tensor(calciumversion=calciumversion,keepraw=False)
...
model_CC           = CCA(n_components=n_components,scale = False, max_iter = 1000)
# model_CC           = PLSCanonical(n_components=n_components,scale = False, max_iter = 1000)
...
# for ises,ses in tqdm(enumerate([sessions[0]]),total=nSessions,desc='Fitting CCA model'):    # iterate over sessions
    ...
    for imf in range(nmodelfits):
        ...
        for istim,stim in enumerate(np.unique(ses.trialdata['stimCond'])): # loop over orientations 
            ...
            # Fit CCA MODEL:
            model_CC.fit(X,Y)
            # ccaweights = model_CC.x_loadings_[:nsampleneurons,:]
            # plsweights = model_CC.x_loadings_[:nsampleneurons,:]
            # for i in range(n_components):
            #     plt.scatter(ccaweights[:,i],plsweights[:,i])

            weights_CCA[:,0,ises,istim,imf] = np.mean(np.abs(model_CC.x_loadings_[:nsampleneurons,:]),axis=0)
            weights_CCA[:,1,ises,istim,imf] = np.mean(np.abs(model_CC.x_loadings_[nsampleneurons:,:]),axis=0)

            weights_CCA[:,2,ises,istim,imf] = np.mean(np.abs(model_CC.y_loadings_[:nsampleneurons,:]),axis=0)
            weights_CCA[:,3,ises,istim,imf] = np.mean(np.abs(model_CC.y_loadings_[nsampleneurons:,:]),axis=0)

            # weights_CCA[:,0,ises,istim,imf] = np.mean(np.abs(model_CC.x_weights_[:nsampleneurons,:]),axis=0)
            # weights_CCA[:,1,ises,istim,imf] = np.mean(np.abs(model_CC.x_weights_[nsampleneurons:,:]),axis=0)

            # weights_CCA[:,2,ises,istim,imf] = np.mean(np.abs(model_CC.y_weights_[:nsampleneurons,:]),axis=0)
            # weights_CCA[:,3,ises,istim,imf] = np.mean(np.abs(model_CC.y_weights_[nsampleneurons:,:]),axis=0)
            ...
            if do_cv_cca:#Implementing cross validation
                ...
                for ikf,(train_index, test_index) in enumerate(kf.split(X)):
                    ...
                    X_c, Y_c = model_CC.transform(X_test,Y_test)
