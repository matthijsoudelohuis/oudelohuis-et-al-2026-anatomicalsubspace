...
# for ises,ses in tqdm(enumerate([sessions[0]]),total=nSessions,desc='Fitting CCA model'):    # iterate over sessions
    ...
    for imf in range(nmodelfits):
        ...
        for istim,stim in enumerate(np.unique(ses.trialdata['stimCond'])): # loop over orientations 
            ...
            if fit_fast: 
                x_load, y_load, cancorr = cca_svd(X, Y, n_components)
            ...
            if do_cv_cca: #Implementing cross validation
                ...
                corr_test = np.full((n_components,params['kfold']),np.nan)
...
model_CCA.fit(X,Y)
model_CCA.x_loadings_
model_CCA.y_loadings_
x_loadings, y_loadings, s = cca_svd(X, Y, 20)

# plt.plot(x_loadings[:,0],model_CCA.x_loadings_[:,0],'o')
# plt.plot(x_loadings[:,1],model_CCA.x_loadings_[:,1],'o')
# plt.plot(x_loadings[:,2],model_CCA.x_loadings_[:,2],'o')
...
my_savefig(fig,figdir,'CCA_V1PM_labeled_ratioloadings_%dsessions_%s' % (nSessions,varversion))
...
# for ises,ses in tqdm(enumerate([sessions[0]]),total=nSessions,desc='Fitting CCA model'):    # iterate over sessions
    ...
    for iapl, arealabelpair in enumerate(arealabelpairs):
        ...
        for istim,stim in enumerate(np.unique(ses.trialdata['stimCond'])): # loop over orientations 
            ...
            [g,_] = CCA_subsample(X,Y,nN=Nsub,resamples=nmodelfits,kFold=params['kfold'],prePCA=None,n_components=np.min([n_components,nsampleneurons]))
...
sns.despine(top=True,right=True,offset=1,trim=True)
# my_savefig(fig,figdir,'CCA_V1PM_pops_labeled_testcorr_%dsessions' % (nSessions))
