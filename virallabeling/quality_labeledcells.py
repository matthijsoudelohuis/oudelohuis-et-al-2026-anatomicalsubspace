...
#%% Compute respmat: (necessari for gOSI tuning metric)
...
fig, (ax1,ax2) = plt.subplots(2,1,figsize=(5*cm,4.2*cm),sharex=True)

sns.histplot(data=celldata,x='frac_red_in_ROI',stat='probability',hue='redcell',
             palette=get_clr_labeled(),binwidth=0.05,ax=ax1,alpha=1)
             
sns.histplot(data=celldata,x='frac_red_in_ROI',stat='probability',hue='redcell',
             palette=get_clr_labeled(),binwidth=0.05,ax=ax2,alpha=1)
...
ax2.set_ylabel('fraction of cells',horizontalalignment='left',verticalalignment='center')
ax1.set_ylabel('')
# ax1.set_xlabel('ROI overlap')
ax2.set_xlabel('ROI overlap')
plt.tight_layout()
...
plt.tight_layout()
my_savefig(fig,figdir,'Histogram_ROIoverlap_%dcells_%dsessions' % (len(celldata),nsessions))
...
plt.tight_layout()
# my_savefig(fig,figdir,'Scatter_Overlap_Twoways_%dcells_%dsessions' % (len(celldata),nsessions))


#%%
sesdata = pd.DataFrame()
sesdata['ncells']         = celldata.groupby(["session_id","arealabel"])['arealabel'].count()

#%% Bar plot of number of labeled cells per area:
# arealabels = celldata['arealabel'].unique()
arealabels = ['V1unl','V1lab','PMunl','PMlab','ALunl']
figlabels = arealabeled_to_figlabels(arealabels)
clrs_arealabels = ['grey','red','grey','red','grey']
fig, ax = plt.subplots(figsize=(3*cm,5*cm))
sns.barplot(data=sesdata,x='arealabel',y='ncells',palette=clrs_arealabels,ax=ax,errorbar='se',order=arealabels)
sns.stripplot(data=sesdata,x='arealabel',y='ncells',color='k',ax=ax,size=3,alpha=0.5,jitter=0.2,order=arealabels)
ax.set_yscale('log')
...
ax.set_xticklabels(figlabels,rotation=45,ha='right')
my_savefig(fig,figdir,'nLabeled_area_%dsessions' % nsessions)

#%% Select only cells nearby labeled cells to ensure fair comparison of quality metrics:
celldata = pd.concat([ses.celldata[filter_nearlabeled(ses,radius=params['radius'])] for ses in sessions]).reset_index(drop=True)
# celldata = pd.concat([ses.celldata for ses in sessions]).reset_index(drop=True)
...
# fields = ["radius","noise_level","event_rate","meanF"]
# fieldlabels = ['cell radius','noise level','event rate','mean F']
fields = ["radius","noise_level","event_rate","meanF","gOSI"]
fieldlabels = ['cell radius','noise level','event rate','mean F', 'gOSI']

nfields = len(fields)
fig,axes   = plt.subplots(1,nfields,figsize=(nfields*4*cm,3.5*cm))

celldataclip = copy.deepcopy(celldata)

for i in range(nfields):
    ax = axes[i]
    celldataclip[fields[i]] = np.clip(celldata[fields[i]],np.nanpercentile(celldata[fields[i]],0.5),
                                      np.nanpercentile(celldata[fields[i]],99.5))
    sns.violinplot(data=celldataclip,y=fields[i],x="arealabel",hue="arealabel",palette=clrs_arealabels,
                   ax=axes[i],order=arealabels,inner='quart',legend=False)

    ax.set_ylim(np.nanpercentile(celldataclip[fields[i]],[0.1,99.9]))
    ax.set_xticklabels(figlabels,rotation=45,ha='right')
    ax.set_xlabel('')
    ax.set_ylabel(fieldlabels[i])
    ax.set_ylim(0,ax.get_ylim()[1]*1.1)

    for ipair,pair in enumerate(pairs):
        x1 = arealabels.index(pair[0])
        x2 = arealabels.index(pair[1])

        g  = np.nanmean(celldata.loc[celldata['arealabel']==pair[1],fields[i]]) / np.nanmean(celldata.loc[celldata['arealabel']==pair[0],fields[i]])
        print('{0}: ratio = {1:.1f}%'.format(fields[i],(g-1)*100))

        y, h, col = ax.get_ylim()[1]*0.95, ax.get_ylim()[1]*0.02, 'k'
        ax.plot([x1, x1, x2, x2], [y, y+h, y+h, y], lw=1.5, c=col)
        ax.text((x1+x2)*.5, y+h, '%+1.1f%%' % ((g-1)*100), ha='center', va='bottom', color=col, fontsize=6)

        # annotator = Annotator(ax, [pair], data=celldata, x="arealabel", y=fields[i], order=arealabels)
        # annotator.configure(test='Mann-Whitney', text_format='star', loc='inside',verbose=False)
        # annotator.apply_and_annotate()

        # ax.set_title('%s\n (%+1.1f%%)' % (fields[i],(g-1)*100),fontsize=7) #fields[i])
...
# plt.xlabel(r'Area')
# plt.tight_layout()
# my_savefig(fig,figdir,'nCellpose_area_%dsessions' % nsessions)
...
# plt.xlabel(r'Area')
# plt.tight_layout()
# my_savefig(fig,figdir,'nSuite2p_area_%dsessions' % nsessions)
...
# plt.xlabel(r'Area')
# plt.tight_layout()
# my_savefig(fig,figdir,'Frac_responsive_area_%dsessions' % nsessions)
...
# my_savefig(fig,figdir,'Frac_labeled_area_%dsessions' % nsessions)

# #%% Bar plot of number of labeled cells per area:
# fig, ax = plt.subplots(figsize=(4*cm,3.5*cm))
