...
#%% Show Performance R2 across stim and sessions:
fig, axes = plt.subplots(1,1,figsize=(3*cm,3.6*cm))
ax = axes
handles = []
labels = []
# normalized = True
normalized = False
params['multcomp_method'] = 'bonferroni'
...
for i in range(4):
    ax.plot(i,np.nanmean(data,axis=1)[i],color=clrs_arealabelpairs.flatten()[i],marker='o',linestyle='',alpha=1,markersize=4)
...
for icomb, comb in enumerate(combinations):
    ...
    group2 = df[comb[1]]
    # t_stat, p_value = stats.ttest_rel(group1, group2, nan_policy='omit')
    t_stat, p_value = stats.wilcoxon(group1, group2, nan_policy='omit')
...
for icomb, comb in enumerate(combinations):
    if pvalues_corrected[icomb] < 0.05:
        if pvalues_corrected[icomb]<0.0001:
            print(f"Comparison: {comb[0]} vs {comb[1]}, p-value: {pvalues_corrected[icomb]:1.3e}")
        else:
            print(f"Comparison: {comb[0]} vs {comb[1]}, p-value: {pvalues_corrected[icomb]:.4f}")
        x1 = np.where(labels == comb[0])[0][0]
        x2 = np.where(labels == comb[1])[0][0]
        add_stat_annotation(ax, x1, x2, np.nanpercentile(data,70) + icomb*0.001, pvalues_corrected[icomb], lw=0.8,h=0)
...
version = 'FF_original'
filename = 'RRR_Joint_looped_FF_2026-08-12_00-27-54'
...
R2_toplot = np.reshape(R2_cv[:,:,isubspace],(nsourcearealabelpairs,ntargetarealabelpairs,params['nSessions']*params['nStim'],params['nT']))
...
for isa,sourcearea in enumerate(sourcearealabelpairs):
    for ita,targetarea in enumerate(targetarealabelpairs):
        handles.append(shaded_error(params['t_axis'],R2_toplot[isa,ita,:,:],error='sem',
                                    color=clrs_arealabelpairs[isa,ita],alpha=0.3,ax=ax,linewidth=1))#clrs[iapl],alpha=0.3,ax=ax))
...
my_savefig(fig,figdir,'perf_looped_time_%s' % (version))
...
#%% Get the R2 ratio data for different subspaces:
# noise_constant = 1e-3
noise_constant = 0
clipval = 1e-3
ymin = 0.9
...
fig,axes = plt.subplots(1,3,figsize=(3.3*3*cm,3.7*cm))
for isubspace in range(3):
    ...
    ax.set_ylabel('performance ratio')
sns.despine(fig=fig, top=True, right=True, offset = 3)
my_savefig(fig,figdir,'RRR_looped_time_ratio_subspaces_%s' % (version))
