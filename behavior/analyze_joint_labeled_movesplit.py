# -*- coding: utf-8 -*-
"""
This script analyzes a multi-area calcium imaging
...
my_savefig(fig,figdir,'rank_R2_movesplit_labunl_FF_ExampleSession%d' % ises)
...
# plt.tight_layout()
# sns.despine(fig=fig,trim=False,top=True,right=True)
# my_savefig(fig,figdir,'RRR_joint_cvR2_labunl_FB_examplesession_%d' % ises)
...
plt.tight_layout()
sns.despine(fig=fig,trim=False,top=True,right=True)
my_savefig(fig,figdir,'rank_R2_movesplit_labunl_FB_examplesession_%d' % ises)
...
for istate in range(2):
    ...
    ydata = np.transpose(ydata,(2,0,1)).reshape(params['nranks'],-1)
    handles.append(shaded_error(np.arange(params['nranks']),ydata.T,ax=ax,error='sem',linewidth=1,
                                color=clrs_arealabelpairs[idxs[0]-1],alpha=0.3))
...
for icontrast,contrast in enumerate(contrasts):
    for istate in range(2):
        ratiodata = (R2_cv_FF[contrast[0]][istate] / R2_cv_FF[contrast[1]][istate]).flatten()
        ratiodata = np.nan_to_num(ratiodata,nan=np.nan,posinf=np.nan,neginf=np.nan)
        handle = ax.errorbar(x=istate,y=np.nanmean(ratiodata),yerr=2*np.nanstd(ratiodata)/np.sqrt(len(ratiodata)),
        ...
        if p < 0.05:
            ax.annotate(get_sig_asterisks(p),xy=(istate,np.nanmean(ratiodata)+0.06),fontsize=8,ha='center',color='red')
        if istate == 1:
...
ax_nticks(ax,4)
ax.set_xticks([0,1],statelabels)

ax = axes[1]
handles = []
for icontrast,contrast in enumerate(contrasts):
    for istate in range(2):
        ratiodata = (R2_cv_FB[contrast[0]][istate] / R2_cv_FB[contrast[1]][istate]).flatten() 
        ratiodata = np.nan_to_num(ratiodata,nan=np.nan,posinf=np.nan,neginf=np.nan)
        handle = ax.errorbar(x=istate,y=np.nanmean(ratiodata),yerr=2*np.nanstd(ratiodata)/np.sqrt(len(ratiodata)),
                             color=clrs[icontrast],capsize=3,elinewidth=1,marker='o',markersize=5)[0]
        h,p = stats.wilcoxon(ratiodata-1,nan_policy='omit')
